...
class Map():
    ...
    def redraw(self, save=False):
        ...
        if x_scat.size > 0 and y_scat.size > 0:
            margin = 1.0
            self.ax.set_xlim(min(x_scat) - margin, max(x_scat) + margin)
            self.ax.set_ylim(min(y_scat) - margin, max(y_scat) + margin)
        if self.draw_path:
    ...
    def add_data(self, data: datapoint.DataPoint):
        ...
        for d in data_iter:
            ...
            if self.draw_path and hasattr(d, 'path_id') and d.path_id is not None:
...
if __name__ == "__main__":
    from slam.common.datapoint import DataPoint

    m = Map()
    for i in range(20):
        dp = DataPoint(x=i, y=i * 2, path_id=0)
        m.add_data(dp)
    m.redraw()
